...
case class OppijanumeroRekisteriClient(config: Config) {
  ...
  def findOppijatNoSlaveOids(oids: List[Oid]): IO[List[SuppeatOppijaHenkilöTiedot]] =
  ...
  def findOppijatByHetusNoSlaveOids(hetus: List[String]): IO[List[SuppeatOppijaHenkilöTiedot]] =
  ...
  private def findOnrOppijatByOids(oids: List[Oid]): IO[List[OppijaNumerorekisteriPerustiedot]] =
  ...
  private def withSlaveOids(onrOppijat: Iterable[OppijaNumerorekisteriOppija]): IO[List[LaajatOppijaHenkilöTiedot]] =
    Nondeterminism[Task].gather(onrOppijat.toSeq.map(complementWithSlaveOids)) // TODO
