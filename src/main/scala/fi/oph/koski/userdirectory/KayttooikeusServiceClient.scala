...
case class KäyttöoikeusServiceClient(config: Config) {
  ...
  def findKäyttöoikeusryhmät: IO[List[KäyttöoikeusRyhmä]] = http.get(uri"/kayttooikeus-service/kayttooikeus/KOSKI")(parseJson[List[KäyttöoikeusRooli]]).flatMap { roolit =>
